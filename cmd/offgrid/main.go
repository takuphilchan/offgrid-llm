...
func printHelp() {
	...
	fmt.Printf("  %s$%s offgrid benchmark tinyllama-1.1b-chat.Q4_K_M\n", brandMuted, colorReset)
	fmt.Println()
	fmt.Printf("  %s# Embeddings for semantic search and RAG%s\n", brandMuted, colorReset)
	fmt.Printf("  %s$%s offgrid download all-minilm-l6-v2 F16\n", brandMuted, colorReset)
	fmt.Printf("  %s$%s curl -X POST http://localhost:11611/v1/embeddings \\\n", brandMuted, colorReset)
	fmt.Printf("       -d '{\"model\":\"all-minilm-l6-v2\",\"input\":\"Hello world\"}'\n")
	fmt.Println()
