...
static gboolean saved=FALSE;
static gboolean shortcut(GtkWidget *widget,GdkEventKey *event,gpointer data){
    if((event->state&GDK_CONTROL_MASK)&&event->keyval==GDK_KEY_s){saved=TRUE;return TRUE;}return FALSE;
}
static gboolean input(GIOChannel *channel,GIOCondition condition,gpointer data){
    ...
    else if(strcmp(line,"status\n")==0){puts(saved?"SAVED":"NOT_SAVED");fflush(stdout);}
    else if(strcmp(line,"checked\n")==0){puts(gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(checkbox))?"CHECKED":"UNCHECKED");fflush(stdout);}
...
int main(int argc,char **argv){
    ...
    checkbox=gtk_check_button_new_with_label("Include sources");gtk_box_pack_start(GTK_BOX(box),checkbox,FALSE,FALSE,0);
    ...
    g_signal_connect(window,"destroy",G_CALLBACK(gtk_main_quit),NULL);g_signal_connect(window,"key-press-event",G_CALLBACK(shortcut),NULL);gtk_widget_show_all(window);gtk_window_present(GTK_WINDOW(window));gtk_widget_grab_focus(entry);
