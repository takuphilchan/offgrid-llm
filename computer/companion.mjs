...
try {
  ...
  console.log(`A separate browser starts at ${origin} and may access pages on that site only. OffGrid may read its pages. Changes require approval. No personal profile, saved login or downloads are used. Ctrl+C closes this session immediately. Sessions expire after 10 minutes and are not resumed.`);
  ...
  await session.start({origin,code,networkMode});
