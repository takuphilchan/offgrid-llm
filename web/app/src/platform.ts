...
export type DesktopBridge = {
  discoverComputerApps?: (request:{workspace:string}) => Promise<{state:string;targets?:{id:string;title:string;driver:string}[];code?:string}>;
  startComputerApp?: (request:{workspace:string;target:string;approvalMode:'ask_every_time'|'scoped_changes'|'full_task'}) => Promise<{state:string;target?:{id:string;origin:string};code?:string}>;
  ...
  startComputerBrowser?: (request:{origin:string;workspace:string;networkMode?:'direct'|'trusted-vpn';uploadGrant?:string;approvalMode:'ask_every_time'|'scoped_changes'|'full_task'}) => Promise<{state:string;code?:string;target?:{id:string;origin:string}}>;
