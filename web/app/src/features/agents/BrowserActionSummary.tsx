...
export function BrowserActionSummary({tool,args,steps}:{tool:string;args:Record<string,unknown>;steps:AgentStep[]}) {
 ...
 for(const step of [...steps].reverse()) {
  try {
   ...
   if((view.observation_id??view.observation?.id)!==args.observation_id || !Array.isArray(view.elements)) continue;
   ...
   if(typeof element?.name==='string' && element.name) target=element.name;
 ...
 return <div className="browser-action-summary"><h2>{browserActionLabel(locale,tool)}</h2><dl>
  ...
  {typeof args.shortcut==='string' && <><dt>{text.change}</dt><dd>{args.shortcut.replace('_',' ')}</dd></>}
